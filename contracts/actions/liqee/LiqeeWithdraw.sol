...
/// @title Withdraw a token from Liqee
contract LiqeeWithdraw is ActionBase, LiqeeHelper {
    ...
    error LiqeeRedeemError();
    error LiqeeUnderlyingRedeemError();
    ...
    function executeAction(
        ...
        emit ActionEvent("LiqeeWithdraw", logData);
    ...
    function executeActionDirect(
        ...
        logger.logActionDirectEvent("LiqeeWithdraw", logData);
    }
    ...
    /// @notice Withdraws a underlying token amount from liqee
    /// @dev Send type(uint).max to withdraw whole balance
    /// @param _iTokenAddr iToken address
    ...
    function _withdraw(
        ...
        if (_amount == type(uint256).max) {
            _amount = _iTokenAddr.getBalance(address(this));
            // if (IIToken(_cTokenAddr).redeem(_amount) != NO_ERROR) {
            //     revert CreamRedeemError();
            // }
            IIToken(_iTokenAddr).redeem(address(this) , _amount);
        } else {
            ...
            // }
            IIToken(_iTokenAddr).redeemUnderlying(address(this), _amount);
        }
