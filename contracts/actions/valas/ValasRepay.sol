...
import "./helpers/ValasHelper.sol";

/// @title Repay a token a user borrowed from an Valas market
contract ValasRepay is ActionBase, ValasHelper {
    ...
    function executeAction(
        ...
        emit ActionEvent("ValasRepay", logData);
    ...
    function executeActionDirect(
        ...
        logger.logActionDirectEvent("ValasRepay", logData);
    }
    ...
    /// @notice User paybacks tokens to the Valas protocol
    ...
    function _payback(
        ...
        // IValasLendingPool lendingPool = getLendingPool(_market);
        IValasLendingPool lendingPool = IValasLendingPool(_market);

        uint256 maxDebt = getWholeDebt(
            VALAS_POOL_ADDRESS_PROVIDER,
            _tokenAddr,
            _rateMode,
            _onBehalf
        );
