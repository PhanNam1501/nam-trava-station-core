...
contract TravaNFTHeuristicFarmingPolish is
    ActionBase,
    TravaNFTHeuristicFarmingHelper
{
    ...
    function executeAction(
        ...
        (uint256 level, bytes memory logData) = _polishNFT(
            params.vault,
            params.ids,
            params.level
        );

        emit ActionEvent("TravaNFTHeuristicFarmingPolish", logData);
    ...
    function executeActionDirect(
        ...
        Params memory params = parseInputs(_callData);
        (, bytes memory logData) = _polishNFT(
            params.vault,
            params.ids,
            params.level
        );
        logger.logActionDirectEvent("TravaNFTHeuristicFarmingPolish", logData);
    }
    ...
    function _polishNFT(
        ...
        IFarming(_vault).polishNFT(_ids, _level);
