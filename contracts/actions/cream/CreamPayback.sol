...
/// @title Payback a token a user borrowed from Cream
contract CreamPayback is ActionBase, CreamHelper {
    ...
    function executeAction(
        ...
        emit ActionEvent("CreamPayback", logData);
    ...
    function executeActionDirect(
        ...
        logger.logActionDirectEvent("CreamPayback", logData);
    }
    ...
    /// @notice Payback a borrowed token from the Cream finance
    /// @dev Amount type(uint).max will take the whole borrow amount
    /// @param _cTokenAddr Address of the vToken we are paying back
