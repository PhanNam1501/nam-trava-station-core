...
/// @title Repay a token a user borrowed from Venus
contract VenusRepay is ActionBase, VenusHelper {
    ...
    function executeAction(
        ...
        emit ActionEvent("VenusRepay", logData);
    ...
    function executeActionDirect(
        ...
        logger.logActionDirectEvent("VenusRepay", logData);
    }
    ...
    /// @notice Repay a borrowed token from the Venus protocol
    /// @dev Amount type(uint).max will take the whole borrow amount
    /// @param _vTokenAddr Address of the vToken we are paying back
    ...
    function _payback(
        ...
        uint256 maxDebt = IVToken(_vTokenAddr).borrowBalanceCurrent(_onBehalf);
