...
describe("swapETHforToken", function () {
    ...
    //   await tx2.wait();
      ...
      const swapETHforToken = new Action(
        "swapETHforToken",
        process.env.SWAPTOKENFORTOKEN,
        ["address", "address[]", "uint256", "uint256","address","address", "uint256"],
        [router, [weth, tokenAddr], amountETHIn, amountOutMin ,from, referrer, deadline]
      );
  
      // const calldata = traveSupply.encodeForRecipe()[0];
      const callData = swapETHforToken.encodeForDsProxyCall()[1];
  
      console.log("callData", callData);
  
      const swapETHforTokenContract = await hre.ethers.getContractAt(
        "swapETHforToken",
        process.env.SWAPETHFORTOKEN
      );

      console.log("swapETHforToken_address",swapETHforTokenContract.address);
      ...
      let tx = await proxy["execute(address,bytes)"](
        swapETHforTokenContract.address,
        callData,
        {
          gasLimit: 20000000,
        }
