...
describe("swapTokenforToken", function () {
    this.timeout(150000);
    console.log("hello");
    it("Test swapTokenforToken", async () => {
      ...
      const amountIn = hre.ethers.BigNumber.from("10");
    ...
    //   await tx2.wait();
      ...
      const swapTokenforToken = new Action(
        "swapTokenforToken",
        process.env.SWAPTOKENFORTOKEN,
        ["address", "address[]", "uint256", "uint256","address","address", "uint256"],
        [router, [tokenAAddr, tokenBAddr], amountIn, amountOutMin ,from, referrer, deadline]
      );
  
      // const calldata = traveSupply.encodeForRecipe()[0];
      const callData = swapTokenforToken.encodeForDsProxyCall()[1];
  
      console.log("callData", callData);
  
      const swapTokenforTokenContract = await hre.ethers.getContractAt(
        "swapTokenforToken",
        process.env.SWAPTOKENFORTOKEN
      );

      console.log("swapTokenforToken_address",swapTokenforTokenContract.address);
      ...
      let tx = await proxy["execute(address,bytes)"](
        swapTokenforTokenContract.address,
        callData,
        {
          gasLimit: 20000000,
        }
