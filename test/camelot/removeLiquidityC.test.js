...
describe("removeLiquidityC", function () {
    this.timeout(150000);
    console.log("hello");
    ...
    //   await tx2.wait();
      ...
      const removeLiquidityC = new Action(
        "removeLiquidityC",
        process.env.ADDLIQUIDITYC,
        ["address", "address", "address","address", "uint256","uint256","uint256","uint256","address"],
        [router, tokenAAddr, tokenBAddr, tokenPair, liquidity, amountAmin, amountBmin, deadline, from]
      );
  
      // const calldata = traveSupply.encodeForRecipe()[0];
      const callData = removeLiquidityC.encodeForDsProxyCall()[1];
  
      console.log("callData", callData);
  
      const removeLiquidityCContract = await hre.ethers.getContractAt(
        "removeLiquidityC",
        process.env.REMOVELIQUIDITYC
      );

      console.log("removeliquidityC_address",removeLiquidityCContract.address);
      ...
      let tx = await proxy["execute(address,bytes)"](
        removeLiquidityCContract.address,
        callData,
        {
          gasLimit: 20000000,
        }
