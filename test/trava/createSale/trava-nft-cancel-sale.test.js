...
describe("Test Pancakeswap", async function () {
  ...
  it("Test create sell nft , after cancel it", async () => {
    let triggerCallData = [];
    let actionsCallData = [];
    let subData = [];
    let actionIds = [];
    ...
    const cancelSaleAction = new Action(
      "NFT Cancel Sale",
      process.env.TRAVA_NFT_CANCLE_SALE_ADDRESS,
      ["uint256", "address"],
      [tokenID, accA.address]
    );

    const callDataCreateSale = createSaleAction.encodeForRecipe()[0];
    const callDataCancelSale = cancelSaleAction.encodeForRecipe()[0];

    actionsCallData.push(callDataCreateSale);
    actionsCallData.push(callDataCancelSale);
    ...
    const subdataCancelSaleTokenID = abiCoder.encode(["uint256"], [tokenID]);
    const subdataCancelSaleTo = abiCoder.encode(["address"], [accA.address]);

    actionIds = [
      keccak256("TravaNFTCreateSale").substr(0, 10),
      keccak256("TravaNFTCancelSale").substr(0, 10),
    ];
